...
현재 컬렉터는 `claude_code` / `codex` / `obsidian` 3종뿐. 사용자 본인이 남기는 흔적은 훨씬 넓다 — shell, 다른 AI 도구, 글쓰기 앱, 커뮤니케이션, 행동 신호. 본 sprint에서는 **Tier 1 (file-mirror 패턴 재활용 가능, PII 가벼움) 4종을 ship**하고 Tier 2~4 (10종)는 골격 + spec 만 남기고 후속 sprint 에서 단계 진행.

### 본 sprint 범위 (Tier 1, 4개)
...
- Tier 1 4개 컬렉터 모두 `pytest tests/test_<name>_mirror.py` 통과.
- `pytest tests/` 전체 회귀 통과.
- `synapse-memory daily --quick` 로 Tier 1 4개 stage 모두 ≤2s 끝남 (각 home 디렉토리 비어있어도 graceful).
- `docs/reference.md` 의 "외부 데이터 수집기" 표가 7종 (기존 3 + 신규 4) 으로 확장.
