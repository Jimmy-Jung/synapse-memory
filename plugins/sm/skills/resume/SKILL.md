...
- **모델**: 가장 높은 최신 모델(현재 Claude Opus 계열 최상위)을 사용한다.
- **effort**: 최고 단계(xhigh/max)로 실행한다.
- **오케스트레이션**: 무거운 단계(회사 조사·프로젝트 매칭)는 **Ultracode** 로 병렬 fan-out 한다. Codex 에서 실행 시 동급 최고 모델 + 최고 reasoning + 멀티에이전트 오케스트레이션을 사용한다.
- 본 스킬은 vault(Obsidian) 컨텍스트에서 동작하며 경로는 vault 루트 기준이다.

## 입력

- **회사**: 이름 또는 slug (예: `쿠팡`, `coupang`).
- **공고**: JD 텍스트 또는 URL(예: Wanted/회사 채용 페이지). 없으면 사용자에게 요청한다.
...
2. `20_Reference/Companies/<slug>/` 폴더를 만든다. (기존 flat `Companies/<slug>.md` 가 있으면 폴더로 옮겨 통합.)
3. **소스 자료 점검**: 위 "소스 자료"(Entities/Profile/Concepts/Insights + 기존 이력서/자소서)에 경력 자료가 있는지 확인한다.
   - **있으면** → 그걸 1차 자료로 Phase 1 진행.
   - **첫 사용이고 이력서·자소서·프로젝트 카드가 전혀 없으면 → 인터뷰 모드**: `references/interview-guide.md` 를 따라 구조화 인터뷰로 경력 사실(회사·프로젝트 STAR·역량·학력·링크)을 수집해 `Companies/<slug>/interview-notes.md` 로 정리한 뒤 Phase 1 로 넘어간다. 인터뷰로 드러난 프로젝트는 자동으로 카드화하지 않고, 승격 여부를 사용자에게 안내한다.
4. 소스 자료는 **읽기/링크 전용**이다. **새 프로젝트 폴더/카드를 만들지 않는다** — 기존 카드만 링크한다.
...
- Ultracode: 개요/제품/엔지니어링문화/문화·인재상/JD 를 별도 에이전트로 병렬 조사 후 통합.

## Phase 2 — 프로젝트 매칭

- `Entities/Projects/*.md` 를 스캔(glob)하고 회사 키워드와 매칭되는 카드를 랭킹한다. (Ultracode: 매칭 에이전트 fan-out 가능)
- `Profile/*` 에서 voice·강점, `Concepts/`·`Insights/` 에서 보강 근거를 함께 끌어온다.
- analysis.md §7(키워드→프로젝트 표)을 채운다. 각 행에 `[[Entities/Projects/<file>]]` 링크.
...
- 회사 매칭 프로젝트/역량을 상단 배치.
- 회사·학력·자격증 등 고정 정보는 기준 이력서에서 가져오되 강조 순서만 회사에 맞춘다.
...
20_Reference/Companies/<slug>/
  ...
  resume-draft.md   # 프로젝트 링크 + 섹션 재료 (검토 게이트)
  resume.md         # 최종 경력기술서
```
