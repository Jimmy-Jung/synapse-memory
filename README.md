...
Synapse Memory는 새 노트와 대화 기록을 자동으로 모아 서로 연결된 wiki로 정리하고,
그 wiki로 질문에 답합니다. v2.0.0부터 페이지들은 단순 링크가 아니라 typed relation
(`uses`/`part_of`/`decided_in`/`supersedes` 등)으로 연결된 지식그래프이며, 검색이
이 관계를 실제로 따라가며 답을 찾습니다. 그래서 사용자는 원본 파일을 뒤지기보다
...
설치 파일은 아래 링크에서 받아줘.
https://github.com/Jimmy-Jung/synapse-memory/releases/download/v2.0.1/SynapseMemory-v2.0.1-macos-installer.zip
...
1. [SynapseMemory-v2.0.1-macos-installer.zip][installer-zip]을 다운로드합니다.
...
| 다른 프로젝트에 sm 컨텍스트 hook 등록 | `/sm:setup` | `$setup` | `synapse-memory setup` |
...
`ask --save`로 저장한 답변은 `<vault>/Insights/YYYY/MM/`에 insight 엔티티로 쌓이고
...
| wiki 구조 자동수정 + schema 검증 (관계 domain/range, coverage) | `synapse-memory lint --now` |
...
### v2.0.0 — 단일 Entity 모델 + 온톨로지

v2.0.0은 wiki를 typed relation 지식그래프로 재구축했습니다. 페이지는 6종 엔티티
(project/company/concept/insight/log/profile)로 통일되고, `src/synapse_memory/schema.yaml`
하나가 타입·필드·관계의 단일 스키마입니다.

- **typed relation**: `uses`, `part_of`(이행), `broader`(개념 계층, 이행),
  `decided_in`, `supersedes`, `same_as`(대칭). 검색이 관계 타입별로 이웃을
  확장하고, 질의 의도에 따라 역방향("X를 uses하는 project는?")도 답합니다.
- **시간 무효화**: 새 사실이 `supersedes`를 선언하면 이전 페이지는 자동으로
  무효화(`status: superseded` + `t_invalid`)되고, 기본 검색은 현재 유효한
  사실만 봅니다. `recall`은 supersedes 이력을 시간순으로 따라갑니다.
- **개념 분류**: concept는 `kind`(technology/tool/algorithm/methodology)로
  하위분류되고, 결정(decision)은 insight/log로 기록됩니다.
- **그래프 건강 지표**: `synapse-memory doctor`가 `typed_relation_coverage`,
  `legacy_related_residual`, `orphan_ratio`를 보여줍니다. ingest는 무타입 연결을
  차단해 그래프 품질이 회귀하지 않습니다.

설계·검증 근거는 [specs/022-ontology-completion/review.md](specs/022-ontology-completion/review.md)를 참고하세요.
...
- hook이 설치되어 있어도 현재 디렉터리가 registry에 없으면 의도적으로 아무 것도
  주입하지 않습니다. 해당 repo에서 `synapse-memory setup`을 한 번 실행하세요.
...
[installer-zip]: https://github.com/Jimmy-Jung/synapse-memory/releases/download/v2.0.1/SynapseMemory-v2.0.1-macos-installer.zip
