...
데모 시나리오: `me draft-resume <회사>` (또는 `/synapse-resume <회사>`) — 회사 맞춤 이력서 자동 생성.
...
**왜 분리하나?** AI 비서 · 세컨드 브레인 · 클론의 가치는 *LLM 대화 컨텍스트 안*에서 발생합니다. 터미널에서 직접 `ask`를 부르면 답은 받지만 후속 질문에 컨텍스트가 끊깁니다. 그래서 대화형 endpoint(`ask`, `me *`) 는 TTY 직접 호출 시 3초 안내 후 진행 — `SYNAPSE_FROM_AGENT=1` env 가 있으면 즉시 통과 (slash command markdown 이 자동으로 설정). 배치 endpoint 는 항상 자유.
