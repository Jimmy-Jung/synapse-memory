...
`/sm:ask`, `/sm:recall`, `/sm:resume` 같은 짧은 명령으로 자기 자료를
다시 사용할 수 있습니다.
...
그 뒤에는 `/sm:assistant`를 부르면 현재 상태를 보고 오늘 할 일을 1-3개로
줄여서 제안합니다.
...
설치 파일은 아래 링크에서 받아줘.
https://github.com/Jimmy-Jung/synapse-memory/releases/download/v0.8.1/SynapseMemory-v0.8.1-macos-installer.zip
...
1. [SynapseMemory-v0.8.1-macos-installer.zip][installer-zip]을 다운로드합니다.
2. zip을 열고 `installer/SynapseMemory-Installer.command`를 실행합니다.
3. 안내에 따라 Obsidian vault를 선택하고 환경 점검을 마칩니다.
4. Claude Code 또는 Codex에서 `/sm:doctor`로 상태를 확인합니다.
...
| --- | --- |
| 환경 점검 | `/sm:doctor` |
| 자동 복구 가능한 문제 수정 | `/sm:fix` |
| 새 노트와 대화 기록 정리 | `/sm:daily` |
| 내 자료에 질문 | `/sm:ask "질문"` |
| 예전에 한 생각 회상 | `/sm:recall "주제"` |
| 의사결정 도움 받기 | `/sm:decide "상황"` |
| 회사 맞춤 이력서 초안 | `/sm:resume <회사>` |
| 오늘 할 일 추천 | `/sm:assistant` |
...
[installer-zip]: https://github.com/Jimmy-Jung/synapse-memory/releases/download/v0.8.1/SynapseMemory-v0.8.1-macos-installer.zip
