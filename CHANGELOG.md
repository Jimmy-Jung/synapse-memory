...
- **CQ10 관계 근거 추적 완성**: ingest가 관계별 인용을 실제 입력과 대조하고,
  출처 식별자·원본 byte 구간·문자 위치·SHA-256을 저장한다. 페이지 갱신 시 기존 근거를
  보존하고 중복을 제거한다. 기존 근거 미기록 관계는 그대로 읽을 수 있다.
- **로컬 원문 확인**: `entity provenance <type:slug> <관계> <대상>`이 로컬 mirror의
  원문을 해시로 검증해 출력한다. 변경·삭제·경로 이탈·조회 한도 초과를 구분하며,
  터미널 제어문자는 escape한다. JSON 출력도 지원한다.
- **원문 접근 경계**: Vault에는 인용문을 복제하지 않고, `ask`에는 근거 위치와
  미기록 상태만 전달한다. 원문 조회는 읽기 전용이며 AI provider를 호출하지 않는다.
- **예상 실패 해소**: CQ10 검사를 실제 수집→저장→CLI 원문 확인 경로로 교체했다.
  과거 원문 자동 재처리 없이 새 수집부터 근거 기록을 지원한다.
