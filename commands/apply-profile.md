...
### 6. Hook context cache 갱신

후보 파일 마감까지 끝나면 Bash 도구로 다음 명령을 실행합니다.

```bash
synapse-memory context render
```

이 단계는 `Profile.md` / `DecisionPatterns.md` 변경분을 Claude Code
SessionStart hook이 즉시 읽는 `~/.synapse/context/rendered.md`에 반영합니다.
`synapse-memory sync`와 달리 등록 프로젝트의 `AGENTS.md` / `CLAUDE.md` marker는
다시 쓰지 않습니다.

`context render`가 실패하면 적용된 Profile 변경은 rollback하지 말고, 실패 메시지를
결과 요약에 포함해 사용자가 같은 명령을 재실행할 수 있게 안내합니다.
...
- 반영된 카테고리·패턴 목록 (있으면)
- 마감된 후보 파일 경로
- context cache 갱신 결과 (`~/.synapse/context/rendered.md` 또는 실패 메시지)
...
- `context render`는 승인 반영 후 stale hook context를 없애기 위한 필수 후처리. marker 파일까지 갱신해야 할 때만 별도로 `/sm:sync`를 안내.
