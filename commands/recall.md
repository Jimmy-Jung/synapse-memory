...
기본은 의미 거리(distance) 기반 AI provider 통합 답변입니다. `--timeline` 또는 `--by time` 옵션을 붙이면 vault Card 를 period_end 내림차순으로 분기·월 그룹화한 시간순 회상이 됩니다 (외부 LLM 호출 없이 로컬에서 생성).

옵션:
- `--timeline` 시간순 분기 그룹 (권장 — 회상 본연의 경험)
- `--by time` `--timeline` 별칭
- `--by distance` provider 통합 답변 (기본)
