...
| "작년 결정 이유 시간순으로 보여줘" | `me what-did-i-think "..."` | `/synapse-recall` |
| "결정에 도움이 필요해" | `me decide "..."` | `/synapse-decide` |
| "회사 맞춤 이력서 초안" | `me draft-resume <회사>` | `/synapse-resume` |
| "내 성향·결정 패턴 후보 추출" | `me update-profile [--facts-only]` | (자동) |
...
synapse-memory me generate resume --input company_id=examplecorp --language en
```
...
synapse-memory me draft-resume examplecorp --top-k 6 --model sonnet
```
...
synapse-memory me what-did-i-think "샘플회사B" --hybrid
```
