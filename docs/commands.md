...
| "Obsidian 노트와 Claude 대화를 격리 폴더에 모아줘" | `collect obsidian` / `collect claude-code` | (자동) |
| "같은 프로젝트 자료끼리 묶어줘" | `cluster scan` → `cluster classify` | (자동) |
| "묶인 자료를 한 장 카드로 요약해줘" | `card generate` | (자동) |
| "내가 만든 카드 한번 보고 싶다" | `card list` / `card show` / `card new` | (Obsidian에서) |
| "카드를 검색 가능하게 만들어줘" | `rag index [--rebuild] [--include-raw]` | (자동) |
...
| "이번 답변이 좋았어 / 별로였어" | `feedback {accept,reject}` | `/synapse-feedback` |
| "이번 달 비용 얼마 나왔나?" | `cost summary [--days N]` | `/synapse-cost` |
| "마스킹 정확도 측정" | `eval golden` | — |

> 자세한 옵션과 출력 예시는 각 명령 절을 참고하세요. *왜* 이 명령이 있는지는 [for-everyone/how-it-works.md](for-everyone/how-it-works.md)에서 통증별로 설명합니다.
...
`ask`, `persona what-did-i-think`, `persona decide`, `persona draft-resume`, `persona update-profile` 다섯 endpoint 는 사용자가 *터미널에서 직접* 호출했을 때 다음과 같이 동작합니다:
...
```bash
synapse-memory card show sample-ios-app
synapse-memory card show examplecorp --type company
```
...
synapse-memory feedback card sample-ios-app --reject "SwiftUI 주제에는 부적합"
...
| 대상 | 용도 |
|---|---|
| `last` | 직전 `ask` / `persona what-did-i-think` / `persona decide` 답변의 citation 대상에 피드백 |
...
synapse-memory ask "샘플회사B 경험" --hybrid
```
...
### `persona generate <recipe>` (007-me-recipes)

```bash
synapse-memory persona generate weekly_report --input period=2026-W19
synapse-memory persona generate weekly_report --input period=2026-W19 --rag-mode hybrid
synapse-memory persona generate journal --input date=2026-05-12
synapse-memory persona generate brainstorm --input topic="시간관리"
synapse-memory persona generate resume --input company_id=examplecorp --language en
```
...
### `persona recipes list` · `persona recipes show <recipe>`

```bash
synapse-memory persona recipes list                # builtin + user recipe 표
synapse-memory persona recipes list --json         # machine-readable envelope
synapse-memory persona recipes show weekly_report  # input_schema·rag_filter·system_prompt preview
```

### `persona draft-resume`

```bash
synapse-memory persona draft-resume examplecorp
synapse-memory persona draft-resume examplecorp --top-k 6 --model sonnet
```

CompanyCard와 매칭되는 ProjectCard를 바탕으로 회사 맞춤 이력서 초안을 vault에 작성합니다.
007-me-recipes 도입 이후 내부적으로는 `persona generate resume --input company_id=…` 와
동일한 generator pipeline 을 사용하지만 외부 stdout / exit code / 저장 경로는 보존됩니다.

### `persona what-did-i-think`

```bash
synapse-memory persona what-did-i-think "TCA 아키텍처"
synapse-memory persona what-did-i-think "AI 코딩 도구 사용 경험" --top-k 8
synapse-memory persona what-did-i-think "클린 아키텍처" --timeline
synapse-memory persona what-did-i-think "이직 고민" --by time --limit 10
synapse-memory persona what-did-i-think "샘플회사B" --hybrid
```
...
```bash
synapse-memory persona decide "다음 분기에 어디에 시간을 투자할까?"
```

Profile, DecisionPatterns, RAG 검색 결과를 함께 사용해 의사결정 초안을 제안합니다.

### `persona update-profile`

```bash
synapse-memory persona update-profile
synapse-memory persona update-profile --facts-only --sample-lines 200
```
...
### 진행률 — `daily` 실행 중 표시되는 정보

`daily`는 클러스터 단위로 진행 상황을 한 줄씩 출력합니다.

```text
=== [classify] ===
  [1/27] kakaobank-2026 ... ok (8.4s)
  [2/27] notion-design ... ok (6.1s)
=== [generate] ===
  [1/14] kakaobank-2026 (company) ... ok (94.2s)
  [2/14] notion-design (project) ... 실패: timeout (62.0s)
```

동시에 `~/.synapse/run/daily.status.json`에 atomic write로 진행 상태를 기록합니다. stdout이 부모 프로세스(예: AI agent, ECC harness)에 캡처되어 가려진 상황에서도 이 파일을 읽으면 현재 stage·클러스터·경과 시간을 즉시 확인할 수 있습니다.

### `daily-status`

```bash
synapse-memory daily-status                # 사람이 읽기 좋은 형식
synapse-memory daily-status --json         # JSON 원본
synapse-memory daily-status --watch        # 2초 간격으로 변화 추적
synapse-memory daily-status --watch --interval 1
```
...
state:    running
pid:      38092
started:  2026-05-13T01:56:00+00:00
updated:  2026-05-13T02:18:30+00:00
stage:    generate (4/7)
item:     kakaobank-2026 (company) [8/27, 29%]
completed: collect_claude_code, collect_obsidian, classify
```

종료 코드: 진행 중 또는 정상 종료(`done`)는 `0`, 실패(`failed`) 또는 status 파일 없음은 `1`, `--watch` 중 Ctrl-C는 `130`.

Claude Code 데스크탑 / Codex 등 stdout이 즉시 노출되지 않는 환경에서는 이 명령(또는 `cat ~/.synapse/run/daily.status.json`)을 polling해서 진행 상황을 확인하세요.
