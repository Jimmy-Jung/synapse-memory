...
다음 소스를 자동으로 mirror합니다. mirror/collect 단계에서는 모두
...
History, VS Code Local History, `git_self` collector는 개인정보·운영 잡음 대비 실사용
가치가 낮아 제거됐습니다.
...
| 위치 | 의미 |
| --- | --- |
| `~/.synapse/private/` | 원본 mirror와 처리 중간 파일. 내 Mac 안에만 둡니다. |
...
| `Entities/Projects/` · `Entities/Companies/` | Synapse가 만든 프로젝트·회사 엔티티 |
| `Concepts/` | 기술·도구·알고리즘·방법론 등 개념 엔티티 (`kind`로 분류) |
| `Insights/YYYY/MM/` · `Logs/YYYY/MM/` | 통찰·활동 로그 엔티티 |
