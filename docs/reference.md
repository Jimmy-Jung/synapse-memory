...
| 새 자료 정리 | `/sm:daily` | `$daily` | `synapse-memory daily` |
...
`daily`는 Claude Code/Codex raw 로그를 mirror한 뒤 wiki ingest와 lint를 한 번에 실행합니다.

실행 단계는 `collect_claude_code → collect_codex → ingest → lint`입니다. `--only`,
`--skip`, `--resume-from`은 이 네 단계만 받습니다.
...
1. 아직 wiki 페이지가 충분히 만들어지지 않았습니다. `daily`를 실행합니다.
2. 노트가 `10_Active/<회사>/<프로젝트>/` 같은 프로젝트 폴더에 모여 있지 않습니다.

## 관계의 원문 근거 확인

`X uses Y` 같은 관계의 근거 문장은 로컬에서 직접 조회합니다. AI provider를 호출하거나
조회 결과를 Vault에 저장하지 않습니다.

```bash
synapse-memory entity provenance project:alpha uses python
synapse-memory entity provenance project:alpha uses python --vault /path/to/vault --json
```

첫 인자는 출발 페이지의 `type:slug`, 마지막 인자는 관계에 저장된 `slug` 또는
`type:slug`입니다. 새 ingest는 관계별 인용이 실제 입력에 있는지 대조한 후
`relation_evidence`에 출처 식별자, 원본 byte 구간, 추출 텍스트 문자 구간과 SHA-256을
저장합니다. 원문 문장은 이 메타데이터에 복사하지 않습니다.

조회 시 로컬 mirror의 같은 구간을 읽고 해시가 일치할 때만 문장을 출력합니다.
뒤에 대화가 추가되어도 기존 byte 구간은 조회할 수 있지만, 수정·삭제·compact로
구간이 바뀌면 `changed` 또는 `missing` 상태로 알립니다. 기록된 범위가 64 MiB를 넘거나
문장이 2,000자를 넘으면 자동 열람하지 않습니다. 일치 여부는 원문 존재를 확인하며,
그 문장이 관계를 충분히 뒷받침하는지는 사용자가 함께 판단해야 합니다.

기존 관계에 근거가 없으면 `근거 미기록`으로 표시합니다. 과거 원문을 자동 재처리하거나
페이지 전체의 출처를 모든 관계의 근거로 대신 붙이지 않습니다. `ask`에는 기록된 위치와
미기록 상태만 전달하며, 원문을 읽거나 provider에 추가 전송하지 않습니다.

종료 코드는 조회 성공·미기록이면 `0`, 원문 불일치·접근 불가이면 `1`, 페이지·관계 지정
오류이면 `2`입니다. JSON 출력은 근거별 위치·상태·검증된 문장을 담으며 미기록은 빈 배열입니다.
터미널 제어문자와 방향 제어문자는 가시적인 escape로 출력하고, 일반 출력의 개행·탭은
유지합니다. JSON을 파싱하면 검증된 원문 문장을 그대로 얻을 수 있습니다.
...
`daily`나 `persona ingest`는 Profile 후보를 바로 확정하지 않고 `MemoryInbox`에 둡니다. 후보 파일은 `MemoryInbox/{YYYY}/{MM}/`에 정리됩니다.
...
`MemoryInbox`와 `DailyReports`는 `{YYYY}/{MM}/` 하위에 생성됩니다. 과거 flat 구조(`MemoryInbox/Profile-YYYY-MM-DD.md` 등)를 쓰던 vault는 `migrate-folders` 명령으로 옮길 수 있습니다.
...
History, VS Code Local History, `git_self` collector는 지원하지 않습니다.
...
재인제스트 절차는 [Re-ingest Runbook](reingest-runbook.md)을 따릅니다. 이 절차는 provider
호출과 vault 쓰기가 발생하므로 자동으로 실행하지 않습니다.
