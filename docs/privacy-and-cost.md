...
| 작업 | 비용 감각 |
| --- | --- |
| `/synapse-doctor` | 무료에 가까움 |
| `/synapse-fix` | 대부분 로컬 작업 |
| `synapse-memory daily --quick` | 적은 비용, 첫 체험용 |
| `synapse-memory daily` | 노트 양에 따라 비용과 시간이 커질 수 있음 |
| `/synapse-ask` | 질문마다 외부 AI 호출 가능 |
| `/synapse-resume` | 이력서 초안 생성 시 외부 AI 호출 |
| `/synapse-decide` | 상황 판단 시 외부 AI 호출 |
...
- 불필요한 draft와 오래된 MemoryInbox 후보는 `/synapse-cleanup`으로 정리합니다.
