...
관계별 근거 추적은 ingest 입력에 있는 문장을 대조한 뒤, `relation_evidence`에 상대적인
출처 식별자·byte/문자 위치·해시만 저장합니다. 원문 인용을 Vault 메타데이터에 복제하지
않습니다. `ask`도 이 위치 정보만 사용합니다. `entity provenance`는 사용자가 지정한 관계의
원문을 로컬 mirror에서 확인해 터미널에 출력하는 읽기 전용 명령이며, provider 호출이나
자동 저장을 하지 않습니다. 명령의 출력에는 원문이 들어갈 수 있으므로 외부 공유 여부는
사용자가 결정합니다.

## 자동 수집 vs opt-in 컬렉터

외부 데이터 수집기는 두 그룹으로 나뉩니다.
...
- 첫 실행은 `synapse-memory ingest-audit --source codex --limit 50`으로 예상 호출 수를 확인한 뒤 `daily`를 실행합니다.
- raw mirror가 많으면 `backfill --batch-size`와 `--max-batches`로 작은 배치부터 검증합니다.
