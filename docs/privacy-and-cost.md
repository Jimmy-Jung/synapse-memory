...
| 개발 환경 활동 mirror (shell history, Cursor / Continue / Aider, VS Code Local History) | `~/.synapse/private/` | 전송하지 않음 |
| 개인 메모 mirror (Apple Notes, Day One) | `~/.synapse/private/` | 전송하지 않음 |
| macOS 시스템 mirror (Calendar, 브라우저 history, Screen Time) | `~/.synapse/private/` | 전송하지 않음 |
| iMessage `chat.db` mirror (Full Disk Access 부여 시) | `~/.synapse/private/` | 전송하지 않음 |
| Gmail Sent mirror (opt-in 시) | `~/.synapse/private/` | 전송하지 않음 |
| 본인 git commit mirror (opt-in 시) | `~/.synapse/private/` | 전송하지 않음 |
| Apple Health export mirror (drop-in 시) | `~/.synapse/private/` | 전송하지 않음 |
...
원본(raw) mirror는 `~/.synapse/private/`(0700)에 격리되어 외부 AI로 직접 나가지
않습니다. 외부 전송은 질문에 필요한 요약 카드와 사용자가 승인한 자료로 한정됩니다.
...
- Obsidian vault, Apple Notes, Day One
- macOS Calendar, 브라우저 history(Chrome/Safari/Arc), Screen Time

**Opt-in 또는 권한 부여 필요** — 사용자가 명시적으로 활성화한 경우에만:

| 컬렉터 | 활성화 방법 | 비활성화 방법 |
| --- | --- | --- |
| `imessage` | macOS 시스템 설정 → 개인정보 보호 → Full Disk Access 에 Synapse Memory 추가 | `SYNAPSE_IMESSAGE_DISABLE=1` |
| `gmail_sent` | `SYNAPSE_GMAIL_ENABLE=1` + OAuth credentials 등록 | 환경변수 제거 |
| `git_self` | `SYNAPSE_GIT_SELF_ROOTS="~/Documents/GitHub"` 형태로 지정 | 환경변수 제거 |
| `apple_health` | `~/Downloads/export*.zip` 파일 드롭 (또는 `SYNAPSE_HEALTH_DROP=...`) | zip 파일 제거 |

모든 수집은 `~/.synapse/private/`(0700) 내부에만 mirror 되고 외부 AI 에는 직접 전송되지
않습니다. 외부 전송은 항상 요약 카드 + 마스킹 + 사용자 승인 경로를 거칩니다.
...
`daily` 같은 수집/통합 작업은 로컬 임베딩 없이 **대화 단위로 외부 AI(claude CLI)를 호출**합니다.
...
이하는 전체 본문을 1회 통합하고, 40,000자를 넘고 120,000자 이하인 문서는
20,000자 이내 샘플만 1회 통합합니다. 120,000자를 넘는 초대형 문서는 외부 AI에
보내지 않고 skip하며, 같은 파일을 무한 재시도하지 않도록 watermark를 전진시킵니다.
