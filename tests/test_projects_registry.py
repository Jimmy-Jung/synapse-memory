...
def test_save_registry_writes_json_sidecar_for_hooks(tmp_path: Path) -> None:
    registry = tmp_path / "projects.yaml"
    entries = [
        ProjectEntry(
            path=Path("/proj/a"),
            target="both",
            registered_at=datetime.date(2026, 6, 11),
            last_sync=datetime.date(2026, 6, 12),
            state="active",
        )
    ]
    ...
    assert payload["projects"][0]["path"] == "/proj/a"
    assert payload["projects"][0]["state"] == "active"
...
def test_upsert_inserts_new_and_updates_existing(tmp_path: Path) -> None:
    ...
    target_a = next(e for e in after_update if e.path == Path("/proj/a"))
...
def test_mark_stale_updates_state(tmp_path: Path) -> None:
    ...
    target_a = next(e for e in after if e.path == Path("/proj/a"))
    target_b = next(e for e in after if e.path == Path("/proj/b"))
